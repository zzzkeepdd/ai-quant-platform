...
本项目是本地运行的 OKX 加密货币 AI 量化平台。平台集成 OKX 实时行情、OKX 模拟盘账户/持仓/挂单读取、DefiLlama 链上 TVL 摘要、DeepSeek 行情分析、策略回测/参数迭代和风控守卫。
...
- 安全边界：测试下单仅允许 sandbox 模式；实盘仍只展示建议，需人工确认后再扩展真实执行。
